...
/**
 * Filters a collection by property equality or truthiness.
 * @param {Array<Record<string, unknown> | null | undefined> | null | undefined} value
 * @param {string} property
 * @param {string | number | boolean | null | undefined} [expected]
 * @returns {unknown[]}
 */
...
/**
 * Filters a collection by evaluating a Liquid expression against each item.
 * @param {Array<Record<string, unknown> | null | undefined> | null | undefined} value
 * @param {string} variableName
 * @param {string} expression
 * @returns {Promise<unknown[]>}
 */
export async function where_exp(value, variableName, expression) {
  ...
  for (const item of value) {
    if (await this.evaluate(expression, { [variableName]: item })) {
      output.push(item);
    }
  }
...
/**
 * Builds a site-relative URL using the active Jekyll-style site context.
...
/**
 * Builds an absolute URL using the active Jekyll-style site context.
 * @param {string | null | undefined} value
 * @returns {string}
 */
...
/**
 * Groups a collection by a property path.
 * @param {Array<Record<string, unknown> | null | undefined> | null | undefined} value
 * @param {string} property
 * @returns {{ name: string | number | boolean | null, items: Array<Record<string, unknown> | null | undefined>, size: number }[]}
 */
...
/**
 * Groups a collection by the result of a Liquid expression.
 * @param {Array<Record<string, unknown> | null | undefined> | null | undefined} value
 * @param {string} variableName
 * @param {string} expression
 * @returns {Promise<{ name: string | number | boolean | null, items: Array<Record<string, unknown> | null | undefined>, size: number }[]>}
 */
export async function group_by_exp(value, variableName, expression) {
  if (!Array.isArray(value) || !variableName || !expression || typeof this?.evaluator?.resolveExpression !== "function") {
    return [];
  }
  ...
  for (const item of value) {
    const key = await this.evaluator.resolveExpression(expression, { [variableName]: item }, { applyFilters: false });
    const name = key === null || key === undefined ? null : key;
...
async function includeRelativeTag({ context, engine, expression, resolveArgument, state }) {
  ...
  const snippetValue = await resolveArgument(snippetExpression);
  ...
  const response = await (engine.evaluator?.fetch || engine.fetch)?.(request);
  ...
  return await engine.rewriter.renderFragment(
    template,
    childContext,
    state.runtime,
    nextDepth,
...
/**
 * Installs Jekyll-oriented filters and the `include_relative` tag on a Liquid instance.
 * @param {typeof import("../mod.js").Liquid} Liquid
 * @returns {typeof import("../mod.js").Liquid}
 */
