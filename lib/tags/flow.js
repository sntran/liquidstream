...
function createIfTag(name, invert = false) {
  return {
    ...
    async onEmit(ctx) {
      ...
      const evaluated = await evaluate(ctx, ctx.expression || EMPTY_STRING);
...
export const elsif = {
  ...
  async onEmit(ctx) {
    ...
    const truthy = await evaluate(ctx, ctx.expression || EMPTY_STRING);
    if (frame) {
      frame.truthy = truthy;
    }
  ...
  async onSkip(ctx) {
    ...
    if (await evaluate(ctx, ctx.expression || EMPTY_STRING)) {
      frame.truthy = true;
